// 순수 함수 모음 — 서버(getStaticProps)와 브라우저(필터 변경 시) 양쪽에서 동일하게 사용한다.
// 입력은 lib/data.js의 toClientRows()가 만든 압축 행 배열.

function isoWeekInfo(dateStr) {
  const d = new Date(dateStr + "T00:00:00Z");
  const day = (d.getUTCDay() + 6) % 7; // Mon=0 .. Sun=6
  const monday = new Date(d);
  monday.setUTCDate(d.getUTCDate() - day);
  const sunday = new Date(monday);
  sunday.setUTCDate(monday.getUTCDate() + 6);
  const fmt = (x) =>
    `${String(x.getUTCMonth() + 1).padStart(2, "0")}/${String(x.getUTCDate()).padStart(2, "0")}`;
  return { key: monday.toISOString().slice(0, 10), label: `${fmt(monday)}~${fmt(sunday)}` };
}

const monthOf = (d) => d.slice(0, 7);
const sumPremium = (rows) => rows.reduce((a, r) => a + (r.premium || 0), 0);
// "건수"는 상담(counsel_application) 단위다 — snowflake_sales_export.sql의 그레인은 상담×차량
// 1행이라, 차량이 여러 대인 상담은 여러 행으로 내려온다. 매출(원수보험료) 합계는 차량 단위 그대로
// 다 더하지만(그래야 금액이 맞다), "건수"는 반드시 distinct counselId로 세야 한다.
const distinctCounselCount = (rows) => new Set(rows.map((r) => r.counselId)).size;

function bucketBy(rows, keyFn, labelFn) {
  const map = new Map();
  for (const r of rows) {
    const key = keyFn(r);
    if (!map.has(key)) map.set(key, []);
    map.get(key).push(r);
  }
  const out = [...map.entries()].map(([key, list]) => {
    const premiumSum = sumPremium(list);
    const count = distinctCounselCount(list);
    return {
      key,
      label: labelFn ? labelFn(key, list) : key,
      count,
      premiumSum,
      avgPremium: count > 0 ? Math.round(premiumSum / count) : 0,
    };
  });
  out.sort((a, b) => (a.key < b.key ? -1 : a.key > b.key ? 1 : 0));
  return out;
}

export function filterRows(rows, { dateFrom, dateTo, manager }) {
  return rows.filter((r) => {
    if (dateFrom && r.date < dateFrom) return false;
    if (dateTo && r.date > dateTo) return false;
    if (manager && manager !== "ALL" && r.managerName !== manager) return false;
    return true;
  });
}

export function aggregate(rows) {
  const totals = {
    count: distinctCounselCount(rows),
    joinCompletedCount: distinctCounselCount(rows.filter((r) => r.currentStatus === "JOIN_COMPLETED")),
    pendingCount: distinctCounselCount(rows.filter((r) => r.currentStatus === "ACCUMULATE_PENDING")),
    premiumSum: sumPremium(rows),
    dealerCount: new Set(rows.map((r) => r.dealerKey)).size,
    dateMin: rows.reduce((m, r) => (m === "" || r.date < m ? r.date : m), ""),
    dateMax: rows.reduce((m, r) => (m === "" || r.date > m ? r.date : m), ""),
  };
  totals.avgPremium = totals.count ? Math.round(totals.premiumSum / totals.count) : 0;

  const daily = bucketBy(rows, (r) => r.date);
  const weekly = bucketBy(rows, (r) => isoWeekInfo(r.date).key, (k, l) => isoWeekInfo(l[0].date).label);
  const monthly = bucketBy(rows, (r) => monthOf(r.date));

  // 보험사 x 가입유형 피벗 (원수보험료 합계 + 가입건수). 건수는 상담 단위라 같은 (보험사,가입유형)
  // 조합 안에서 counselId가 겹치면(차량 여러 대) 한 건으로만 센다 — Set으로 모았다가 size로 뺀다.
  const insurerSet = new Set();
  const typeSet = new Set();
  const pivotMap = new Map();
  const pivotCounselMap = new Map();
  for (const r of rows) {
    insurerSet.add(r.insurer);
    typeSet.add(r.joinType);
    const k = r.insurer + "|" + r.joinType;
    pivotMap.set(k, (pivotMap.get(k) || 0) + (r.premium || 0));
    if (!pivotCounselMap.has(k)) pivotCounselMap.set(k, new Set());
    pivotCounselMap.get(k).add(r.counselId);
  }
  const typeOrder = ["CM", "TM", "OFFLINE"];
  const types = [...typeSet].sort((a, b) => typeOrder.indexOf(a) - typeOrder.indexOf(b));
  const insurerRows = [...insurerSet]
    .map((insurer) => {
      const byType = {};
      const byTypeCount = {};
      let total = 0;
      let totalCount = 0;
      for (const t of types) {
        const k = insurer + "|" + t;
        const v = pivotMap.get(k) || 0;
        const c = pivotCounselMap.get(k)?.size || 0;
        byType[t] = v;
        byTypeCount[t] = c;
        total += v;
        totalCount += c;
      }
      return { insurer, byType, byTypeCount, total, totalCount };
    })
    .sort((a, b) => b.total - a.total);
  const typeTotals = types.reduce((acc, t) => {
    acc[t] = insurerRows.reduce((s, row) => s + row.byType[t], 0);
    return acc;
  }, {});
  const typeCountTotals = types.reduce((acc, t) => {
    acc[t] = insurerRows.reduce((s, row) => s + row.byTypeCount[t], 0);
    return acc;
  }, {});
  const insurerPivot = {
    rows: insurerRows,
    types,
    typeTotals,
    typeCountTotals,
    grandTotal: insurerRows.reduce((s, r) => s + r.total, 0),
    grandCount: insurerRows.reduce((s, r) => s + r.totalCount, 0),
  };

  // 딜러별
  const dealerMap = new Map();
  for (const r of rows) {
    if (!dealerMap.has(r.dealerKey)) dealerMap.set(r.dealerKey, []);
    dealerMap.get(r.dealerKey).push(r);
  }
  const dealerRank = [...dealerMap.entries()]
    .map(([, list]) => ({
      dealerName: list[0].dealerName,
      managerName: list[0].managerName,
      group: list[0].group,
      count: distinctCounselCount(list),
      premiumSum: sumPremium(list),
    }))
    .sort((a, b) => b.premiumSum - a.premiumSum);

  // 채널별
  const channelMap = new Map();
  for (const r of rows) {
    if (!channelMap.has(r.channel)) channelMap.set(r.channel, []);
    channelMap.get(r.channel).push(r);
  }
  const byChannel = [...channelMap.entries()]
    .map(([channel, list]) => ({ channel, count: distinctCounselCount(list), premiumSum: sumPremium(list) }))
    .sort((a, b) => b.count - a.count);

  // 딜러유형 그룹별 (실제 business_type 기준) - 배정 딜러 수
  const groupMap = new Map();
  for (const r of rows) {
    if (!groupMap.has(r.group)) groupMap.set(r.group, new Set());
    groupMap.get(r.group).add(r.dealerKey);
  }

  // 비견(비교견적완료) 퍼널 — 이 raw pull은 성사된 건만 담고 있어 "손실 포함 전환율"은 계산할 수 없다.
  // 대신 체결 건 중 비교견적 단계를 거친 비율과, 단계별 평균 소요일을 본다.
  const comparisonRows = rows.filter((r) => r.hasComparison);
  const avg = (arr) => (arr.length ? arr.reduce((s, v) => s + v, 0) / arr.length : null);
  const funnel = {
    comparisonCount: comparisonRows.length,
    comparisonRate: totals.count ? (comparisonRows.length / totals.count) * 100 : 0,
    avgProspectToCompDays: avg(
      comparisonRows.map((r) => r.prospectToCompDays).filter((v) => v != null)
    ),
    avgCompToJoinDays: avg(comparisonRows.map((r) => r.compToJoinDays).filter((v) => v != null)),
  };

  return {
    totals,
    periods: { daily, weekly, monthly },
    insurerPivot,
    dealerRank,
    byChannel,
    groupDealerCount: groupMap,
    funnel,
  };
}

const CONVERTED_STATUSES = new Set(["ACCUMULATE_PENDING", "JOIN_COMPLETED"]);

// 매니저별 신규/갱신 콜(상담 생성월 코호트) 체결률 + 원수보험료.
// callRows: lib/data.js의 loadCallRows() 결과 — "생성일" 기준 필터링.
// premiumRows: toClientRows()가 만든 압축 행(unpack된 것) — "체결일(매출인식일)" 기준 필터링.
// ⚠️ 두 입력의 기간 의미가 다르다 — 콜 쪽은 "그 달에 새로 들어온 상담", 원수보험료 쪽은
// "그 달에 매출로 인식된 건" 이라 같은 期간을 넣어도 서로 다른 상담 집합을 센다.
// "매출"(수수료 매출) 컬럼은 계산식이 아직 확정되지 않아 이 함수에서 만들지 않는다 —
// 화면에서 항상 "-"로 표시한다.
export function aggregateManagerFunnel(callRows, premiumRows, { dateFrom, dateTo }) {
  const calls = callRows.filter((r) => {
    if (dateFrom && r.createdDate < dateFrom) return false;
    if (dateTo && r.createdDate > dateTo) return false;
    return true;
  });
  const premium = premiumRows.filter((r) => {
    if (dateFrom && r.date < dateFrom) return false;
    if (dateTo && r.date > dateTo) return false;
    return true;
  });

  const premiumMap = new Map();
  for (const r of premium) {
    premiumMap.set(r.managerName, (premiumMap.get(r.managerName) || 0) + (r.premium || 0));
  }

  const managerMap = new Map();
  const ensure = (name) => {
    if (!managerMap.has(name)) {
      managerMap.set(name, { managerName: name, newCalls: 0, newDeals: 0, renewalCalls: 0, renewalDeals: 0 });
    }
    return managerMap.get(name);
  };
  for (const r of calls) {
    const m = ensure(r.managerName || "미배정");
    const converted = CONVERTED_STATUSES.has(r.finalStatus);
    if (r.consultType === "갱신") {
      m.renewalCalls += 1;
      if (converted) m.renewalDeals += 1;
    } else {
      m.newCalls += 1;
      if (converted) m.newDeals += 1;
    }
  }
  // 콜 데이터엔 없지만 원수보험료(매출인식) 쪽엔 있는 매니저도 행에 포함시킨다.
  for (const name of premiumMap.keys()) ensure(name);

  const rows = [...managerMap.values()]
    .map((m) => ({
      ...m,
      newRate: m.newCalls ? (m.newDeals / m.newCalls) * 100 : null,
      renewalRate: m.renewalCalls ? (m.renewalDeals / m.renewalCalls) * 100 : null,
      premiumSum: premiumMap.get(m.managerName) || 0,
    }))
    .sort((a, b) => b.premiumSum - a.premiumSum);

  const sumOf = (key) => rows.reduce((s, r) => s + r[key], 0);
  const totals = {
    managerName: "합계",
    newCalls: sumOf("newCalls"),
    newDeals: sumOf("newDeals"),
    renewalCalls: sumOf("renewalCalls"),
    renewalDeals: sumOf("renewalDeals"),
    premiumSum: sumOf("premiumSum"),
  };
  totals.newRate = totals.newCalls ? (totals.newDeals / totals.newCalls) * 100 : null;
  totals.renewalRate = totals.renewalCalls ? (totals.renewalDeals / totals.renewalCalls) * 100 : null;

  const n = rows.length || 1;
  const average = {
    managerName: "인당 평균",
    newCalls: totals.newCalls / n,
    newDeals: totals.newDeals / n,
    renewalCalls: totals.renewalCalls / n,
    renewalDeals: totals.renewalDeals / n,
    premiumSum: totals.premiumSum / n,
    newRate: totals.newRate,
    renewalRate: totals.renewalRate,
  };

  return { rows, totals, average, hasCallData: callRows.length > 0 };
}

// 주유권/지급대기/가입취소처럼 건수가 적은 리스트형 데이터를 기간·매니저로 거르는 공용 함수.
// (건 단위 배열이면 무엇이든 재사용 — 각 항목은 date, managerName 필드를 가진다고 가정)
export function filterListRows(list, { dateFrom, dateTo, manager }) {
  return list
    .filter((r) => {
      if (dateFrom && r.date < dateFrom) return false;
      if (dateTo && r.date > dateTo) return false;
      if (manager && manager !== "ALL" && r.managerName !== manager) return false;
      return true;
    })
    .sort((a, b) => (a.date > b.date ? -1 : a.date < b.date ? 1 : 0));
}

// 주유권 리스트 전용 — filterListRows에 권종별 요약을 더한다.
export function filterGiftRows(giftRows, filters) {
  const list = filterListRows(giftRows, filters);
  const summaryMap = new Map();
  for (const r of list) summaryMap.set(r.giftName, (summaryMap.get(r.giftName) || 0) + 1);
  return {
    summary: [...summaryMap.entries()]
      .map(([giftName, count]) => ({ giftName, count }))
      .sort((a, b) => b.count - a.count),
    list,
  };
}

// 원수보험료 → 매출(수수료) 환산율. 실제 수수료 테이블이 없어 잠정 고정값을 쓴다.
export const REVENUE_RATE = 0.11;

// [체결 지표] 월별 접수/계약(신규·갱신 분리)/전환율/원수보험료/매출액 요약.
// ⚠️ 두 입력의 "월"이 서로 다른 기준이다 — 접수는 상담 생성월(callRows.createdDate),
// 계약/원수보험료는 매출인식(체결)월(rows.date) 기준. 같은 달이어도 서로 다른 상담 집합을 센다.
// 계약 건수는 확정 성사(JOIN_COMPLETED)만 센다 — 지급대기는 원수보험료 합계에는 들어가지만
// "계약"으로는 안 친다(참고 대시보드 실측 검증 기준과 동일).
export function aggregateContractSummary(rows, callRows, { dateFrom, dateTo, manager } = {}) {
  const calls = callRows.filter((r) => {
    if (dateFrom && r.createdDate < dateFrom) return false;
    if (dateTo && r.createdDate > dateTo) return false;
    if (manager && manager !== "ALL" && r.managerName !== manager) return false;
    return true;
  });
  const deals = rows.filter((r) => {
    if (dateFrom && r.date < dateFrom) return false;
    if (dateTo && r.date > dateTo) return false;
    if (manager && manager !== "ALL" && r.managerName !== manager) return false;
    return true;
  });

  const byMonth = new Map();
  const ensure = (m) => {
    if (!byMonth.has(m)) {
      byMonth.set(m, {
        month: m,
        receivedNew: 0,
        receivedRenewal: 0,
        dealsNew: 0,
        dealsRenewal: 0,
        premiumSumNew: 0,
        premiumSumRenewal: 0,
      });
    }
    return byMonth.get(m);
  };

  for (const c of calls) {
    const m = ensure(c.createdDate.slice(0, 7));
    if (c.consultType === "갱신") m.receivedRenewal += 1;
    else m.receivedNew += 1;
  }
  for (const r of deals) {
    const m = ensure(r.date.slice(0, 7));
    const isRenewal = r.consultType === "갱신";
    if (r.currentStatus === "JOIN_COMPLETED" || r.currentStatus === "ACCUMULATE_PENDING") {
      if (isRenewal) m.premiumSumRenewal += r.premium || 0;
      else m.premiumSumNew += r.premium || 0;
    }
    if (r.currentStatus === "JOIN_COMPLETED") {
      if (isRenewal) m.dealsRenewal += 1;
      else m.dealsNew += 1;
    }
  }

  // 원수보험료·매출액은 항상 전체/신규/갱신 3갈래로 함께 보여준다.
  const withDerived = (m) => {
    const received = m.receivedNew + m.receivedRenewal;
    const dealsTotal = m.dealsNew + m.dealsRenewal;
    const premiumSum = m.premiumSumNew + m.premiumSumRenewal;
    return {
      ...m,
      received,
      dealsTotal,
      premiumSum,
      conversionRate: received ? (dealsTotal / received) * 100 : 0,
      revenueNew: m.premiumSumNew * REVENUE_RATE,
      revenueRenewal: m.premiumSumRenewal * REVENUE_RATE,
      revenue: premiumSum * REVENUE_RATE,
    };
  };

  const months = [...byMonth.values()].map(withDerived).sort((a, b) => (a.month < b.month ? 1 : -1));

  const totalsRaw = [...byMonth.values()].reduce(
    (acc, m) => ({
      month: "전체",
      receivedNew: acc.receivedNew + m.receivedNew,
      receivedRenewal: acc.receivedRenewal + m.receivedRenewal,
      dealsNew: acc.dealsNew + m.dealsNew,
      dealsRenewal: acc.dealsRenewal + m.dealsRenewal,
      premiumSumNew: acc.premiumSumNew + m.premiumSumNew,
      premiumSumRenewal: acc.premiumSumRenewal + m.premiumSumRenewal,
    }),
    { receivedNew: 0, receivedRenewal: 0, dealsNew: 0, dealsRenewal: 0, premiumSumNew: 0, premiumSumRenewal: 0 }
  );

  return { months, totals: withDerived(totalsRaw) };
}

// [실적(전체)] 월별 표에서 한 달을 클릭했을 때 보여주는 채널별·상담사별 상세 — 접수/계약/전환율/
// 원수보험료/매출액을 groupBy 기준(channel 또는 managerName)으로 묶는다. "접수"는 콜 생성월이 아니라
// rows(매출인식월 기준, 취소 제외) 건수를 쓴다 — 콜 데이터엔 채널 정보가 없어서 채널별로는 이 방식밖에
// 안 된다. 그래서 이 표의 접수 합계는 월별 표의 접수(콜 생성 기준) 합계와 다를 수 있다.
export function aggregateGroupBreakdown(rows, { dateFrom, dateTo, manager, groupBy = "channel" } = {}) {
  const scoped = rows.filter((r) => {
    if (dateFrom && r.date < dateFrom) return false;
    if (dateTo && r.date > dateTo) return false;
    if (manager && manager !== "ALL" && r.managerName !== manager) return false;
    return true;
  });

  const groupMap = new Map();
  const ensure = (key) => {
    if (!groupMap.has(key)) groupMap.set(key, { key, received: 0, dealsTotal: 0, premiumSum: 0 });
    return groupMap.get(key);
  };
  for (const r of scoped) {
    const g = ensure(r[groupBy] || "기타");
    g.received += 1;
    if (r.currentStatus === "JOIN_COMPLETED") g.dealsTotal += 1;
    if (r.currentStatus === "JOIN_COMPLETED" || r.currentStatus === "ACCUMULATE_PENDING") {
      g.premiumSum += r.premium || 0;
    }
  }

  const totalReceived = scoped.length;
  const groups = [...groupMap.values()]
    .map((g) => ({
      ...g,
      share: totalReceived ? (g.received / totalReceived) * 100 : 0,
      conversionRate: g.received ? (g.dealsTotal / g.received) * 100 : 0,
      revenue: g.premiumSum * REVENUE_RATE,
    }))
    .sort((a, b) => b.received - a.received);

  const totals = groups.reduce(
    (acc, g) => ({
      received: acc.received + g.received,
      dealsTotal: acc.dealsTotal + g.dealsTotal,
      premiumSum: acc.premiumSum + g.premiumSum,
    }),
    { received: 0, dealsTotal: 0, premiumSum: 0 }
  );
  totals.share = 100;
  totals.conversionRate = totals.received ? (totals.dealsTotal / totals.received) * 100 : 0;
  totals.revenue = totals.premiumSum * REVENUE_RATE;

  return { groups, totals };
}

// granularity별 버킷 키. "daily"=날짜 그대로, "weekly"=월요일 시작일, "monthly"=YYYY-MM.
// DB 분석 쪽(lib/dbAggregate.js)도 같은 규칙을 쓰므로 export해서 재사용한다(중복 구현 방지).
export function bucketKey(dateStr, granularity) {
  if (granularity === "monthly") return monthOf(dateStr);
  if (granularity === "weekly") return isoWeekInfo(dateStr).key;
  return dateStr;
}

// [고객 인입 지표] 일/주/월 × 채널 스택 데이터. status가 있으면 그 상태인 건만(예: JOIN_COMPLETED로
// "체결" 차트), 없으면 취소를 제외한 전 건(=최소 지급대기까지 진행된 건)을 "유입" 차트로 쓴다.
export function aggregateDailyByChannel(rows, { dateFrom, dateTo, manager, status, granularity = "daily" } = {}) {
  const filtered = rows.filter((r) => {
    if (dateFrom && r.date < dateFrom) return false;
    if (dateTo && r.date > dateTo) return false;
    if (manager && manager !== "ALL" && r.managerName !== manager) return false;
    if (status && r.currentStatus !== status) return false;
    return true;
  });

  const channels = [...new Set(filtered.map((r) => r.channel))].sort();
  const byBucket = new Map();
  for (const r of filtered) {
    const key = bucketKey(r.date, granularity);
    if (!byBucket.has(key)) byBucket.set(key, {});
    const bucket = byBucket.get(key);
    bucket[r.channel] = (bucket[r.channel] || 0) + 1;
  }

  const data = [...byBucket.keys()].sort().map((date) => {
    const bucket = byBucket.get(date);
    const total = channels.reduce((s, c) => s + (bucket[c] || 0), 0);
    return { date, total, ...bucket };
  });

  return { channels, data };
}

// [회원(딜러) 지표] group(영업채널: 신차딜러 수입/국산·중고차딜러·보험설계사·에이전시)로 필터링해서
// 딜러별/채널별 현황을 만든다.
export function aggregateMembers(rows, { dateFrom, dateTo, channel, manager } = {}) {
  const scoped = rows.filter((r) => {
    if (dateFrom && r.date < dateFrom) return false;
    if (dateTo && r.date > dateTo) return false;
    if (channel && channel !== "ALL" && r.group !== channel) return false;
    if (manager && manager !== "ALL" && r.managerName !== manager) return false;
    return true;
  });

  const dealerMap = new Map();
  for (const r of scoped) {
    if (!dealerMap.has(r.dealerKey)) {
      dealerMap.set(r.dealerKey, {
        dealerKey: r.dealerKey,
        dealerName: r.dealerName,
        group: r.group,
        managerName: r.managerName,
        count: 0,
        premiumSumNew: 0,
        premiumSumRenewal: 0,
      });
    }
    const d = dealerMap.get(r.dealerKey);
    d.count += 1;
    if (r.consultType === "갱신") d.premiumSumRenewal += r.premium || 0;
    else d.premiumSumNew += r.premium || 0;
  }
  const dealers = [...dealerMap.values()]
    .map((d) => ({ ...d, premiumSum: d.premiumSumNew + d.premiumSumRenewal }))
    .sort((a, b) => b.premiumSum - a.premiumSum);

  const byGroupMap = new Map();
  for (const r of scoped) {
    if (!byGroupMap.has(r.group)) byGroupMap.set(r.group, new Set());
    byGroupMap.get(r.group).add(r.dealerKey);
  }
  const byGroup = [...byGroupMap.entries()]
    .map(([group, set]) => ({ group, dealerCount: set.size }))
    .sort((a, b) => b.dealerCount - a.dealerCount);

  return { totalDealers: dealerMap.size, dealers, byGroup };
}

// "YYYY-MM-DD"에 n개월을 더한 날짜. 활동회원 6개월 창(join_at ~ join_at+6개월) 계산용.
function addMonths(dateStr, n) {
  const [y, m, d] = dateStr.split("-").map(Number);
  return new Date(Date.UTC(y, m - 1 + n, d)).toISOString().slice(0, 10);
}

// [회원(딜러) 활동] Business Rule: 가입일(dealerJoinDate) 이후 6개월 이내 계약(가입완료 또는
// 지급대기) 1건 이상이면 "활동회원". 같은 딜러가 6개월 내 여러 건 계약해도 활동회원은 1명.
//
// ⚠️ 알려진 한계: 이 함수는 rows(=매출 쿼리 결과, counsel_application 있는 딜러만 존재)에서
// 딜러를 뽑는다. 한 번도 상담을 만든 적 없는 가입 딜러는 rows에 아예 안 잡혀서 "전체 가입회원"
// 분모가 실제보다 적게 나올 수 있다 — users 테이블 전체를 도는 별도 쿼리가 있어야 정확해진다.
// dealerJoinDate가 없는 딜러(구 CSV/백업 데이터)는 판정 대상에서 제외한다.
export function aggregateDealerActivity(rows, { asOfDate, dateFrom, dateTo } = {}) {
  const dealerMap = new Map();
  for (const r of rows) {
    if (!r.dealerJoinDate) continue;
    if (!dealerMap.has(r.dealerKey)) {
      dealerMap.set(r.dealerKey, {
        dealerKey: r.dealerKey,
        dealerName: r.dealerName,
        joinDate: r.dealerJoinDate,
        contractDates: [],
      });
    }
    if (r.currentStatus === "JOIN_COMPLETED" || r.currentStatus === "ACCUMULATE_PENDING") {
      dealerMap.get(r.dealerKey).contractDates.push(r.date);
    }
  }

  const dealers = [...dealerMap.values()].map((d) => {
    const windowEnd = addMonths(d.joinDate, 6);
    const inWindow = d.contractDates.filter((cd) => cd >= d.joinDate && cd <= windowEnd);
    return {
      ...d,
      cohort: d.joinDate.slice(0, 7),
      windowEnd,
      windowClosed: windowEnd <= asOfDate,
      hasActivity: inWindow.length > 0,
      contractsInWindow: inWindow.length,
    };
  });

  const inRange = (d) => (!dateFrom || d.joinDate >= dateFrom) && (!dateTo || d.joinDate <= dateTo);
  const closedCohort = dealers.filter((d) => d.windowClosed);
  const confirmedActive = closedCohort.filter((d) => d.hasActivity);

  const totals = {
    totalDealers: dealers.length,
    newDealers: dealers.filter(inRange).length,
    currentActiveDealers: dealers.filter((d) => d.hasActivity).length,
    currentActiveRate: dealers.length ? (dealers.filter((d) => d.hasActivity).length / dealers.length) * 100 : 0,
    closedCohortDealers: closedCohort.length,
    confirmedActiveDealers: confirmedActive.length,
    confirmedInactiveDealers: closedCohort.length - confirmedActive.length,
    confirmedActiveRate: closedCohort.length ? (confirmedActive.length / closedCohort.length) * 100 : 0,
  };

  const within = (d, months) => {
    const end = addMonths(d.joinDate, months);
    return d.contractDates.some((cd) => cd >= d.joinDate && cd <= end);
  };
  const cohortMap = new Map();
  for (const d of dealers) {
    if (!cohortMap.has(d.cohort)) cohortMap.set(d.cohort, []);
    cohortMap.get(d.cohort).push(d);
  }
  const cohorts = [...cohortMap.entries()]
    .map(([cohort, list]) => {
      const closed = list.filter((d) => d.windowClosed);
      const closedActive = closed.filter((d) => d.hasActivity);
      const totalContracts = list.reduce((s, d) => s + d.contractsInWindow, 0);
      return {
        cohort,
        dealerCount: list.length,
        within1mo: list.filter((d) => within(d, 1)).length,
        within3mo: list.filter((d) => within(d, 3)).length,
        within6mo: list.filter((d) => d.hasActivity).length,
        confirmedCount: closed.length,
        confirmedActiveRate: closed.length ? (closedActive.length / closed.length) * 100 : null,
        avgContractsPerDealer: list.length ? totalContracts / list.length : 0,
      };
    })
    .sort((a, b) => (a.cohort < b.cohort ? 1 : -1));

  return { totals, cohorts, dealers };
}

// [가입월 × 계약월 교차 조회] "OO월에 가입한 회원 중 OO월에 계약 체결이 있는 회원" 상세 리스트.
// aggregateDealerActivity의 코호트 집계는 숫자만 보여주므로, 실제 명단이 필요할 때 이 함수를 쓴다.
// 계약 인정 기준은 다른 집계와 동일(JOIN_COMPLETED 또는 ACCUMULATE_PENDING).
// "본인계약여부"는 그 계약을 체결한 상담매니저가 지금 선택된(필터바의) 매니저 본인인지 표시한다 —
// manager가 "ALL"이면(전체 관리자 보기) 비교 대상이 없으니 모두 null(해당없음)로 둔다.
export function listJoinCohortContracts(rows, { joinMonth, contractMonth, manager } = {}) {
  const dealerMap = new Map();
  for (const r of rows) {
    if (!r.dealerJoinDate || r.dealerJoinDate.slice(0, 7) !== joinMonth) continue;
    if (!dealerMap.has(r.dealerKey)) {
      dealerMap.set(r.dealerKey, { dealerKey: r.dealerKey, dealerName: r.dealerName, joinDate: r.dealerJoinDate });
    }
  }

  const list = [];
  for (const r of rows) {
    if (!dealerMap.has(r.dealerKey)) continue;
    if (r.date.slice(0, 7) !== contractMonth) continue;
    if (r.currentStatus !== "JOIN_COMPLETED" && r.currentStatus !== "ACCUMULATE_PENDING") continue;
    const dealer = dealerMap.get(r.dealerKey);
    list.push({
      dealerKey: r.dealerKey,
      dealerName: dealer.dealerName,
      joinDate: dealer.joinDate,
      contractDate: r.date,
      channel: r.channel,
      consultType: r.consultType,
      premium: r.premium || 0,
      managerName: r.managerName,
      isSelfManager: !manager || manager === "ALL" ? null : r.managerName === manager,
    });
  }
  list.sort((a, b) => (a.contractDate < b.contractDate ? -1 : a.contractDate > b.contractDate ? 1 : 0));

  // 회원(딜러) 단위로도 묶어준다 — 몇 명이 계약했는지가 궁금할 때(리스트는 계약 건수만큼 나오므로
  // 같은 딜러가 그 달에 2건 계약했으면 2행이 나온다).
  const dealerCount = new Set(list.map((r) => r.dealerKey)).size;

  return { list, dealerCount, joinedDealerCount: dealerMap.size };
}

// [매니저 실적] 선택한 기간(일간=오늘 · 주간=이번주 · 월간=이번달) 안에서 매니저별 접수/계약/전환율/
// 원수보험료(전체·신규·갱신)를 집계한다. aggregateContractSummary와 같은 계산 규칙(계약 건수는
// JOIN_COMPLETED만, 원수보험료는 JOIN_COMPLETED+ACCUMULATE_PENDING)을 매니저별로 나눠서 쓴다.
export function aggregateByManager(rows, callRows, { dateFrom, dateTo } = {}) {
  const calls = callRows.filter((r) => {
    if (dateFrom && r.createdDate < dateFrom) return false;
    if (dateTo && r.createdDate > dateTo) return false;
    return true;
  });
  const deals = rows.filter((r) => {
    if (dateFrom && r.date < dateFrom) return false;
    if (dateTo && r.date > dateTo) return false;
    return true;
  });

  const byManager = new Map();
  const ensure = (name) => {
    if (!byManager.has(name)) {
      byManager.set(name, {
        managerName: name,
        received: 0,
        dealsNew: 0,
        dealsRenewal: 0,
        premiumSumNew: 0,
        premiumSumRenewal: 0,
      });
    }
    return byManager.get(name);
  };

  for (const c of calls) {
    ensure(c.managerName || "미배정").received += 1;
  }
  for (const r of deals) {
    const m = ensure(r.managerName || "미배정");
    const isRenewal = r.consultType === "갱신";
    if (r.currentStatus === "JOIN_COMPLETED" || r.currentStatus === "ACCUMULATE_PENDING") {
      if (isRenewal) m.premiumSumRenewal += r.premium || 0;
      else m.premiumSumNew += r.premium || 0;
    }
    if (r.currentStatus === "JOIN_COMPLETED") {
      if (isRenewal) m.dealsRenewal += 1;
      else m.dealsNew += 1;
    }
  }

  const managers = [...byManager.values()]
    .map((m) => {
      const dealsTotal = m.dealsNew + m.dealsRenewal;
      const premiumSum = m.premiumSumNew + m.premiumSumRenewal;
      return {
        ...m,
        dealsTotal,
        premiumSum,
        conversionRate: m.received ? (dealsTotal / m.received) * 100 : 0,
      };
    })
    .sort((a, b) => b.premiumSum - a.premiumSum);

  return { managers };
}

// [매니저 실적] 보험사 × 가입유형(CM/TM/OFFLINE)별 원수보험료 피벗. manager를 주면 그 매니저
// 체결 건만("본인 담당 현황") — FilterBar 상단 매니저 선택과 같은 필드를 쓴다.
export function aggregateInsurerPivot(rows, { dateFrom, dateTo, manager } = {}) {
  const scoped = rows.filter((r) => {
    if (dateFrom && r.date < dateFrom) return false;
    if (dateTo && r.date > dateTo) return false;
    if (manager && manager !== "ALL" && r.managerName !== manager) return false;
    return true;
  });

  const typeOrder = ["CM", "TM", "OFFLINE"];
  const typeSet = new Set(scoped.map((r) => r.joinType).filter(Boolean));
  const types = [...typeSet].sort((a, b) => typeOrder.indexOf(a) - typeOrder.indexOf(b));

  const pivotMap = new Map();
  for (const r of scoped) {
    const k = r.insurer + "|" + r.joinType;
    pivotMap.set(k, (pivotMap.get(k) || 0) + (r.premium || 0));
  }
  const insurerSet = new Set(scoped.map((r) => r.insurer).filter(Boolean));
  const rowsOut = [...insurerSet]
    .map((insurer) => {
      const byType = {};
      let total = 0;
      for (const t of types) {
        const v = pivotMap.get(insurer + "|" + t) || 0;
        byType[t] = v;
        total += v;
      }
      return { insurer, byType, total };
    })
    .sort((a, b) => b.total - a.total);

  const typeTotals = types.reduce((acc, t) => {
    acc[t] = rowsOut.reduce((s, row) => s + row.byType[t], 0);
    return acc;
  }, {});

  return { rows: rowsOut, types, typeTotals, grandTotal: rowsOut.reduce((s, r) => s + r.total, 0) };
}
