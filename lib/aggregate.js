...
// [고객 인입 지표] 일자 × 채널 스택 데이터. status가 있으면 그 상태인 건만(예: JOIN_COMPLETED로
// "체결" 차트), 없으면 취소를 제외한 전 건(=최소 지급대기까지 진행된 건)을 "유입" 차트로 쓴다.
export function aggregateDailyByChannel(rows, { dateFrom, dateTo, manager, status } = {}) {
  ...
  const data = [...byDate.keys()].sort().map((date) => {
    const day = byDate.get(date);
    const total = channels.reduce((s, c) => s + (day[c] || 0), 0);
    return { date, total, ...day };
  });
