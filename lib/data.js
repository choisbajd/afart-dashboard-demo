...
// Snowflake 동기화(pages/api/cron/sync.js)나 관리자 웹 업로드(pages/api/admin/upload-sales.js)로
// Vercel Blob에 최신 CSV가 올라온다. 아직 둘 다 연동 전이거나(BLOB_READ_WRITE_TOKEN 없음)
// Blob 조회가 실패하면 저장소에 커밋된 data/raw_query.csv(수기 업데이트분)로 자연스럽게 폴백한다.
