...
// 지급대기를 거쳐 가입완료로 넘어간 건인지 — 이런 건은 '지급대기 → 가입완료' 리스트에서
// 별도로 집계하므로, 메인 KPI/실적 집계에서 또 세면 지급대기 때 한 번 + 가입완료 때 한 번
// 이중으로 잡히게 된다.
function wasPendingBeforeCompleted(r) {
  return r.currentStatus === "JOIN_COMPLETED" && !!r.statusHistory && r.statusHistory.includes("ACCUMULATE_PENDING");
}
...
export function toClientRows(rawRows) {
  return rawRows
    .filter(
      (r) =>
        r.contractDate &&
        r.currentStatus !== "JOIN_CANCELLED" &&
        !wasPendingBeforeCompleted(r) &&
        isEligibleDealer(r)
    )
    .map((r) => {
      const f = funnelOf(r);
...
export function toSalesRows(rawRows, { dateFrom, dateTo }) {
  return rawRows
    .filter((r) => {
      ...
      // 실적 대시보드 원수보험료 합계(toClientRows)와 동일한 기준 — 지급대기를 거쳐
      // 가입완료된 건은 '지급대기 → 가입완료' 리스트에서 별도로 보여주니 여기선 제외.
      if (wasPendingBeforeCompleted(r)) return false;
