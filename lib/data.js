...
import { runSalesExportQuery } from "./snowflakeClient";
...
const CALLS_CSV_PATH = path.join(process.cwd(), "data", "calls.csv");
...
const HEADER_MAP = [
  ...
  "dealerJoinDate", // 회원가입일(users.join_at) — 활동회원 판정용. 이 컬럼 없는 CSV엔 없을 수 있음
];
...
export function hasSnowflakeCreds() {
  const { SNOWFLAKE_ACCOUNT, SNOWFLAKE_USERNAME, SNOWFLAKE_USER, SNOWFLAKE_PRIVATE_KEY } = process.env;
  return Boolean(SNOWFLAKE_ACCOUNT && (SNOWFLAKE_USERNAME || SNOWFLAKE_USER) && SNOWFLAKE_PRIVATE_KEY);
}

// 우선순위: ① Snowflake 실시간 조회(방문마다 최신) → ② 실패 시 Vercel Blob의 최근 동기화
// 스냅샷(pages/api/cron/sync.js가 남겨둔 백업) → ③ 그마저 없으면 저장소에 커밋된
// data/raw_query.csv(있다면). ①이 정상 동작하는 한 ②·③은 안전망일 뿐 쓰이지 않는다.
// Snowflake 실시간 연동 이후로는 raw_query.csv를 저장소에 커밋해두지 않으므로, ③도 없으면
// 헤더만 있는 빈 CSV를 돌려줘서(에러로 화면이 죽는 대신) "데이터 없음" 상태로 보이게 한다.
export async function loadRawText() {
  if (hasSnowflakeCreds()) {
    try {
      ...
      console.error("Snowflake 실시간 조회 실패, Blob/로컬 CSV로 폴백:", err.message);
    }
  }
  ...
  try {
    ...
    console.error("로컬 CSV도 없음 — 빈 데이터로 표시:", err.message);
    return HEADER_MAP.join(",") + "\n";
  }
}

export async function loadRawRows() {
  return parseCsv(await loadRawText());
}
...
// ⚠️ 순서 변경 금지: pages/index.js의 getServerSideProps가 r[0]을 "날짜"로 직접 읽어서
// bounds.min/max를 계산한다(unpackRows까지 안 가고 packed 배열을 그대로 씀). 새 필드는
// 반드시 맨 뒤에 추가할 것 — 앞에 끼워넣으면 그 인덱스 계산이 깨진다.
...
// '가입완료' 리스트 — 현재상태가 JOIN_COMPLETED인 실제 건.
export function toJoinCompletedRows(rawRows) {
  return rawRows
    .filter((r) => r.currentStatus === "JOIN_COMPLETED" && isEligibleDealer(r))
    .map((r) => ({
      date: r.contractDate,
...
export function toPendingRows(rawRows) {
  return rawRows
    ...
    .map((r) => ({
      ...
      dealerName: r.dealerName || "미상",
      dealerId: r.dealerId || "",
      // ⚠️ 갱신 광고비 적립 요율 데이터 소스 미확인 — 실제 요율표/컬럼 확인 전까지는
      // 항상 "-"로 표시한다 (적립 전이라는 의미가 아니라 "아직 못 채움"이라는 뜻).
      adRate: null,
    }));
}
...
const CALLS_HEADER_MAP = ["counselId", "createdDate", "managerName", "consultType", "finalStatus"];
...
function parseCallsCsv(text) {
  ...
  for (let i = 1; i < lines.length; i++) {
    const cols = lines[i].split(",");
    if (cols.length < CALLS_HEADER_MAP.length) continue;
    const row = {};
    CALLS_HEADER_MAP.forEach((key, idx) => {
      row[key] = (cols[idx] ?? "").trim().replace(/^"|"$/g, "");
    });
    rows.push(row);
  }
...
// 콜(상담 생성) 데이터 — 신규/갱신 체결률(생성월 코호트 방식) 계산용. scripts/snowflake_calls_export.sql
// 로 뽑은 파일을 data/calls.csv로 저장하면 자동으로 반영된다. 아직 연동 전(파일이 헤더만 있거나
// 없음)이면 빈 배열을 돌려주고, 화면에서 "콜 데이터 준비중"으로 안내한다.
export function loadCallRows() {
  if (!fs.existsSync(CALLS_CSV_PATH)) return [];
  return parseCallsCsv(fs.readFileSync(CALLS_CSV_PATH, "utf-8"));
}
