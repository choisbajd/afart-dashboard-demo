...
// '지급대기 → 가입완료' 전환 리스트 — 지급대기(ACCUMULATE_PENDING)를 거쳐 최종
// 가입완료(JOIN_COMPLETED)로 넘어간 건. 갱신 계약처럼 지급대기로 잡아뒀다가
// 나중에 완료 처리되는 흐름을 추적하기 위한 용도라, 상태이력에 ACCUMULATE_PENDING이
// 한 번이라도 있었던 가입완료 건만 뽑는다.
export function toPendingCompletedRows(rawRows) {
  return rawRows
    .filter((r) => r.currentStatus === "JOIN_COMPLETED" && isEligibleDealer(r))
    .map((r) => {
      const events = parseHistory(r.statusHistory);
      const pending = [...events].reverse().find((e) => e.status === "ACCUMULATE_PENDING");
      if (!pending) return null;
      const last = events[events.length - 1];
      const changedAt = last ? `2026-${last.at}` : r.contractDate;
      return {
        date: changedAt,
        pendingAt: `2026-${pending.at}`,
        changedAt,
        customerName: r.customerName || "",
        phone: r.phone || "",
        managerName: r.managerName || "미배정",
