...
// DB(가망 상담) 분석 전용 쿼리 — 매출 쿼리와 별개 파일. 매출 쿼리의 "최근 90일 활동" 조건이
// 걸려있지 않아서(DB Aging이 오래된 미계약 건도 봐야 하므로) 절대 섞어 쓰지 않는다.
const DB_EXPORT_SQL_PATH = path.join(process.cwd(), "scripts", "snowflake_db_export.sql");
...
// 매출 쿼리와 DB 쿼리가 이 연결 하나를 같이 재사용한다 — 접속/재시도 로직을 중복 구현하지 않는다.
...
// sqlPath를 그대로 실행해 { columns, rows }를 돌려준다. columns는 SELECT에 적힌 순서 그대로이고,
// rows의 각 값은 컬럼명(한글 alias)을 키로 갖는 객체다.
async function runExportQuery(sqlPath) {
...
// scripts/snowflake_sales_export.sql (매출/계약 실적 — 최근 90일 증분 조건 있음, 건드리지 않음)
export async function runSalesExportQuery() {
  return runExportQuery(EXPORT_SQL_PATH);
}

// scripts/snowflake_db_export.sql (DB 유입/활용 분석 — 기간 제한 없이 전체 이력)
export async function runDbExportQuery() {
  return runExportQuery(DB_EXPORT_SQL_PATH);
}
